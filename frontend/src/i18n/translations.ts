export type Language = "pt" | "en";

export const translations = {
  pt: {
    "nav.features": "Recursos",
    "nav.pricing": "Preços",
    "nav.faq": "Perguntas",
    "nav.login": "Entrar",
    "nav.signup": "Criar conta",
    "hero.badge": "Novo · Simulador de compras com IA",
    "hero.title": "Planeje antes de gastar.",
    "hero.subtitle":
      "O Cofrinho te ajuda a simular compras, prever o futuro do seu dinheiro e decidir com confiança. Sem juros escondidos. Sem surpresas.",
    "hero.cta": "Começar a planejar melhor",
    "hero.cta2": "Ver demonstração",
    "hero.trust": "Confiado por mais de 24.000 pessoas em 12 países",
    "features.title": "Tudo que você precisa para decidir com clareza",
    "features.subtitle":
      "Ferramentas elegantes para dominar seu dinheiro do dia a dia.",
    "feat.sim.title": "Simulador de compras",
    "feat.sim.desc":
      "Pergunte “posso comprar isso?” e veja o impacto real nos próximos 6 meses.",
    "feat.forecast.title": "Previsão financeira",
    "feat.forecast.desc":
      "Modelos automáticos projetam seu saldo futuro com base nos seus hábitos.",
    "feat.tracking.title": "Controle de gastos",
    "feat.tracking.desc":
      "Categorização inteligente, filtros e insights personalizados.",
    "feat.wishlist.title": "Lista de desejos",
    "feat.wishlist.desc":
      "Organize o que você quer comprar e descubra o melhor momento.",
    "feat.analytics.title": "Analytics profundo",
    "feat.analytics.desc":
      "Gráficos claros para receitas, despesas, categorias e tendências.",
    "feat.wallets.title": "Múltiplas carteiras",
    "feat.wallets.desc":
      "Cartões, contas, investimentos e dinheiro em espécie em um só lugar.",

    "testimonials.quote1": "Nunca mais comprei por impulso.",
    "testimonials.designer": "Designer",
    "testimonials.quote2": "O simulador salvou minha viagem.",
    "testimonials.engineer": "Engenheira",
    "testimonials.quote3": "Finalmente entendo pra onde vai meu dinheiro.",
    "testimonials.student": "Estudante",

    "pricing.title": "Simples, transparente, sem pegadinhas",
    "pricing.free": "Gratuito",
    "pricing.pro": "Pro",
    "pricing.team": "Família",
    "pricing.month": "/mês",
    "pricing.cta": "Começar agora",
    "pricing.1wallet": "1 carteira",
    "pricing.unlimitedTransactions": "Transações ilimitadas",
    "pricing.basicSupport": "Suporte básico",
    "pricing.value1": "R$ 9,99",
    "pricing.value2": "R$ 19,99",
    "pricing.unlimitedWallets": "Carteiras ilimitadas",
    "pricing.advancedSimulator": "Simulador avançado com IA",
    "pricing.unlimitedGoals": "Metas ilimitadas",
    "pricing.detailedAnalytics": "Análises detalhadas",
    "pricing.upTo5People": "Até 5 pessoas",
    "pricing.sharedWallets": "Carteiras compartilhadas",
    "pricing.familyReports": "Relatórios familiares",

    "faq.title": "Perguntas frequentes",
    "footer.tagline": "Decisões financeiras mais inteligentes, todo dia.",

    "auth.login.title": "Bem-vindo de volta",
    "auth.login.subtitle": "Entre para continuar seu planejamento",
    "auth.signup.title": "Crie sua conta",
    "auth.signup.subtitle": "Comece grátis, sem cartão de crédito",
    "auth.forgot.title": "Recuperar senha",
    "auth.forgot.subtitle": "Enviaremos um link para redefinir sua senha",
    "auth.email": "E-mail",
    "auth.emailPlaceholder": "maria.almeida@email.com",
    "auth.password": "Senha",
    "auth.passwordPlaceholder": "No mínimo 8 caracteres",
    "auth.passwordHint": "Use 8+ caracteres com um mix de letras e números.",
    "auth.name": "Nome",
    "auth.namePlaceholder": "Maria Almeida",
    "auth.remember": "Lembrar de mim",
    "auth.forgot": "Esqueceu a senha?",
    "auth.login.button": "Entrar",
    "auth.signup.button": "Criar conta",
    "auth.forgot.button": "Enviar link",
    "auth.have": "Já tem uma conta?",
    "auth.dont": "Ainda não tem conta?",
    "auth.or": "ou continue com",

    "auth.title": "Saldo",
    "auth.quote":
      "O simulador de compras transformou minhas decisões de compra ansiosas em decisões calmas e confiantes.",
    "auth.loved": "Amado por 24,000+ consumidores conscientes",
    "auth.security": "Protegido com criptografia de ponta a ponta",

    "auth.terms":
      "Ao criar uma conta, você concorda com nossos Termos e Política de Privacidade.",
    "auth.validation.email": "Digite um e-mail válido",
    "auth.validation.password": "A senha deve ter ao menos 8 caracteres",
    "auth.validation.name": "Digite seu nome",
    "auth.validation.match": "As senhas não coincidem",
    "auth.resetSent": "Link enviado! Verifique seu e-mail.",

    // Onboarding
    "onboarding.skip": "Pular introdução",
    "onboarding.step": "Etapa",
    "onboarding.of": "de",
    "onboarding.welcome.title": "Bem-vindo ao Cofrinho",
    "onboarding.welcome.subtitle":
      "Vamos configurar sua conta em alguns passos rápidos para você começar a planejar melhor.",
    "onboarding.welcome.start": "Vamos começar",
    "onboarding.language.title": "Escolha seu idioma",
    "onboarding.language.subtitle":
      "Você pode alterar isso depois nas configurações.",
    "onboarding.currency.title": "Selecione sua moeda",
    "onboarding.currency.subtitle": "Usaremos essa moeda em todo o aplicativo.",
    "onboarding.wallet.title": "Crie sua primeira carteira",
    "onboarding.wallet.subtitle":
      "Uma carteira ajuda a organizar seu dinheiro.",
    "onboarding.wallet.name": "Nome da carteira",
    "onboarding.wallet.namePlaceholder": "Ex: Conta principal",
    "onboarding.wallet.type": "Tipo",
    "onboarding.balance.title": "Qual seu saldo inicial?",
    "onboarding.balance.subtitle":
      "Informe quanto você tem nessa carteira agora.",
    "onboarding.card.title": "Adicionar um cartão",
    "onboarding.card.subtitle":
      "Opcional — você pode adicionar cartões depois.",
    "onboarding.card.number": "Últimos 4 dígitos",
    "onboarding.card.limit": "Limite",
    "onboarding.income.title": "Qual sua renda mensal?",
    "onboarding.income.subtitle": "Isso nos ajuda a fazer previsões precisas.",
    "onboarding.goal.title": "Crie sua primeira meta",
    "onboarding.goal.subtitle": "O que você gostaria de conquistar?",
    "onboarding.goal.name": "Nome da meta",
    "onboarding.goal.namePlaceholder": "Ex: Reserva de emergência",
    "onboarding.goal.target": "Valor alvo",
    "onboarding.finish.title": "Tudo pronto!",
    "onboarding.finish.subtitle":
      "Sua conta está configurada. Vamos planejar suas compras com inteligência.",
    "onboarding.finish.cta": "Ir para o painel",
    "onboarding.welcome.list1": "Simule qualquer compra antes de fazer",
    "onboarding.welcome.list2": "Preveja seu saldo dos próximos meses",
    "onboarding.welcome.list3": "Organize metas e desejos com clareza",
    "onboarding.step1.title": "Escolha seu idioma",
    "onboarding.step1.subtitle": "Você pode mudar depois",
    "onboarding.flag": "🇧🇷",
    "onboarding.flag.subtitle": "Português (BR)",
    "onboarding.step2.title": "Sua moeda principal",
    "onboarding.step2.subtitle": "Usada em toda a plataforma",
    "onboarding.step3.title": "Crie sua primeira carteira",
    "onboarding.step3.subtitle": "Uma conta ou meio de pagamento",
    "onboarding.step3.name": "Nome",
    "onboarding.step4.title": "Saldo inicial",
    "onboarding.step4.subtitle": "Quanto você tem hoje?",
    "onboarding.step5.title": "Adicione um cartão(opcional)",
    "onboarding.step5.subtitle": "Para acompanhar seus gastos",
    "onboarding.step5.nickname": "Apelido",
    "onboarding.step5.number": "Últimos 4",
    "onboarding.step5.limit": "Limite",
    "onboarding.step6.title": "Sua renda mensal",
    "onboarding.step6.subtitle": "Média das últimas semanas",
    "onboarding.step7.title": "Sua primeira meta",
    "onboarding.step7.subtitle": "Vamos ajudar a alcançar!",
    "onboarding.step7.name": "Nome da meta",
    "onboarding.step7.fund": "Reserva de emergência",
    "onboarding.step7.target": "Valor alvo",
    "onboarding.step7.targetDate": "Até quando",
    "onboarding.step8.title": "Tudo pronto!",
    "onboarding.step8.subtitle": "Bem-vindo ao Cofrinho.",
    "onboarding.step8.open":
      "Vamos abrir seu painel e mostrar um rápido tour dos recursos.",
    "onboarding.save": "Salvando...",

    // Tour
    "tour.skip": "Pular tour",
    "tour.restart": "Reiniciar tour",
    "tour.dashboard.title": "Este é seu painel",
    "tour.dashboard.desc":
      "Veja um resumo completo das suas finanças em um só lugar.",
    "tour.wallets.title": "Suas carteiras",
    "tour.wallets.desc":
      "Gerencie cartões, dinheiro, poupança e investimentos.",
    "tour.simulator.title": "Simulador de compras",
    "tour.simulator.desc":
      "O coração do Cofrinho: descubra se você pode comprar algo.",
    "tour.wishlist.title": "Lista de desejos",
    "tour.wishlist.desc":
      "Salve produtos e saiba o melhor momento para comprá-los.",
    "tour.analytics.title": "Análises",
    "tour.analytics.desc":
      "Entenda seus hábitos financeiros com gráficos detalhados.",
    "tour.goals.title": "Metas financeiras",
    "tour.goals.desc": "Defina objetivos e acompanhe seu progresso.",

    // Dashboard
    "dashboard.greeting": "Bom dia,",
    "dashboard.subtitle": "Aqui está o resumo das suas finanças hoje.",
    "dashboard.snapshot": "Aqui está seu resumo",
    "dashboard.balance": "Saldo atual",
    "dashboard.income": "Receita mensal",
    "dashboard.expenses": "Despesas mensais",
    "dashboard.savings": "Economias",
    "dashboard.healthScore": "Saúde financeira",
    "dashboard.healthExcellent": "Excelente",
    "dashboard.healthGood": "Boa",
    "dashboard.healthFair": "Regular",
    "dashboard.healthPoor": "Atenção",
    "dashboard.quickActions": "Ações rápidas",
    "dashboard.addIncome": "Adicionar receita",
    "dashboard.addExpense": "Adicionar despesa",
    "dashboard.transfer": "Transferir",
    "dashboard.simulate": "Simular compra",
    "dashboard.cardsOverview": "Seus cartões",
    "dashboard.walletsOverview": "Carteiras",
    "dashboard.recentTransactions": "Transações recentes",
    "dashboard.upcomingPurchases": "Próximas compras",
    "dashboard.monthlyForecast": "Previsão mensal",
    "dashboard.forecastDesc": "Projeção do seu saldo nos próximos meses",
    "dashboard.vsLastMonth": "vs. mês anterior",
    "dashboard.projection": "Projeção para os próximos 12 meses",
    "dashboard.basedOn": "Baseado em 12 sinais",
    "dashboard.emergencyFund": "Reserva de emergência",
    "dashboard.spendControl": "Controle de gastos",
    "dashboard.diversification": "Diversificação",
    "dashboard.language": "pt-BR",

    // Wallets
    "wallets.title": "Carteiras",
    "wallets.subtitle": "Gerencie todas as suas contas em um só lugar.",
    "wallets.add": "Nova carteira",
    "wallets.total": "Patrimônio total",
    "wallets.type.credit": "Cartão de crédito",
    "wallets.type.debit": "Cartão de débito",
    "wallets.type.cash": "Dinheiro",
    "wallets.type.savings": "Poupança",
    "wallets.type.investment": "Investimentos",
    "wallets.balance": "Saldo",
    "wallets.limit": "Limite",
    "wallets.used": "utilizado",
    "wallets.available": "Disponível",
    "wallets.confirmDelete": "Tem certeza que quer excluir esta carteira?",
    "wallets.deleteFailed": "Não foi possível excluir a carteira.",

    // Transactions
    "transactions.title": "Transações",
    "transactions.subtitle": "Todo o seu histórico financeiro.",
    "transactions.add": "Nova transação",
    "transactions.searchPlaceholder": "Buscar transações...",
    "transactions.type": "Tipo",
    "transactions.income": "Receita",
    "transactions.expense": "Despesa",
    "transactions.category": "Categoria",
    "transactions.wallet": "Carteira",
    "transactions.date": "Data",
    "transactions.status": "Status",
    "transactions.amount": "Valor",
    "transactions.description": "Descrição",
    "transactions.completed": "Concluída",
    "transactions.pending": "Pendente",
    "transactions.scheduled": "Agendada",
    "transactions.noResults": "Nenhuma transação encontrada",
    "transactions.income.added": "Receita adicionada",
    "transactions.expense.added": "Despesa adicionada",
    "transactions.transfer.added": "Transferência realizada",
    "transactions.saved": "Transação salva",
    "transactions.new": "Nova transação",
    "transactions.register": "Registre uma receita, despesa ou transferência",
    "transactions.emergency": "Reserva de emergência",
    "transactions.groceries": "Mercado",
    "transactions.obs": "Observações",
    "transactions.opt": "Opcional",
    "transactions.transfer.recurring": "Transferência recorrente",
    "transactions.transaction.recurring": "Transação recorrente",
    "transactions.recurring.description": "Repetir todo mês",
    "transactions.confirmDelete":
      "Tem certeza que quer excluir essa transação?",
    "transactions.deleteFailed": "Não foi possível excluir a transação.",
    "transactions.movement": "Todas as suas movimentações em um só lugar",

    "category.food": "Alimentação",
    "category.transport": "Transporte",
    "category.housing": "Moradia",
    "category.entertainment": "Lazer",
    "category.health": "Saúde",
    "category.shopping": "Compras",
    "category.education": "Educação",
    "category.salary": "Salário",
    "category.freelance": "Freelance",
    "category.investment": "Investimento",
    "category.subscription": "Assinaturas",
    "category.other": "Outros",

    "sidebar.dashboard": "Painel",
    "sidebar.transactions": "Transações",
    "sidebar.wallets": "Carteiras",
    "sidebar.wishlist": "Desejos",
    "sidebar.simulator": "Simulador",
    "sidebar.analytics": "Analytics",
    "sidebar.goals": "Metas",
    "sidebar.calendar": "Calendário",
    "sidebar.profile": "Perfil financeiro",
    "sidebar.notifications": "Notificações",
    "sidebar.settings": "Configurações",
    "sidebar.faq": "FAQ",
    "sidebar.account": "Conta",
    "sidebar.delete": "Excluir conta",
    "sidebar.logout": "Sair",

    "dash.balance": "Saldo atual",
    "dash.income": "Receita do mês",
    "dash.expenses": "Despesas do mês",
    "dash.savings": "Poupança",
    "dash.health": "Saúde financeira",
    "dash.quick": "Ações rápidas",
    "dash.addIncome": "Receita",
    "dash.addExpense": "Despesa",
    "dash.transfer": "Transferir",
    "dash.simulate": "Simular",
    "dash.recent": "Transações recentes",
    "dash.upcoming": "Próximas compras",
    "dash.forecast": "Previsão do mês",
    "dash.wallets": "Carteiras",

    "sim.title": "Posso comprar isso?",
    "sim.product": "Produto",
    "sim.price": "Preço",
    "sim.installments": "Parcelas",
    "sim.method": "Método de pagamento",
    "sim.card": "Cartão",
    "sim.cash": "Dinheiro",
    "sim.date": "Data desejada",
    "sim.run": "Simular compra",
    "sim.result": "Resultado da simulação",
    "sim.describe":
      "Descreva a compra e veremos o impacto nos próximos 6 meses.",
    "sim.recommend":
      "Recomendação: adie para outubro. Sua margem de segurança fica 42% maior.",

    "goals.title": "Metas financeiras",
    "goals.new": "Nova meta",
    "wishlist.title": "Lista de desejos",
    "wishlist.subTitle": "O que você quer, quando faz sentido comprar",
    "wishlist.new": "Adicionar item",
    "wishlist.buyNow": "Pode comprar",
    "wishlist.wait": "Aguarde um pouco",
    "wishlist.notRec": "Não recomendado",
    "wishlist.run": "Simular",
    "wishlist.load": "Não foi possível carregar sua wishlist.",
    "wishlist.empty": "Sua wishlist está vazia",
    "wishlist.desc":
      "Adicione algo que você quer comprar e descubra quando faz sentido fazer essa compra.",

    "cal.title": "Calendário financeiro",
    "cal.add": "Novo lembrete",
    "notif.title": "Notificações",
    "notif.markAll": "Marcar tudo como lido",
    "notif.empty": "Você está em dia.",
    "notif.viewAll": "Ver todas",

    "settings.title": "Configurações",
    "settings.profile": "Perfil",
    "settings.appearance": "Aparência",
    "settings.notifications": "Notificações",
    "settings.language": "Idioma",
    "settings.theme": "Tema",
    "settings.danger": "Zona de perigo",
    "settings.deleteDesc":
      "Isto apagará permanentemente sua conta e todos os dados.",
    "settings.deleteConfirm": "Tem certeza de que deseja excluir sua conta?",
    "settings.deleteConfirmButton": "Sim, excluir minha conta",
    "settings.deleteCancelButton": "Não, manter minha conta",
    "settings.deleteSuccess": "Sua conta foi excluída com sucesso.",
    "settings.deleteError":
      "Houve um erro ao excluir sua conta. Por favor, tente novamente mais tarde.",
    "settings.adjust": "Ajuste o Cofrinho ao seu jeito",
    "settings.mode": "Claro, escuro ou automático",
    "settings.recommend": "Recomendações de compra",
    "settings.bills": "Contas próximas",
    "settings.goals": "Progresso de metas",
    "settings.reports": "Relatórios mensais",

    "onb.welcome.title": "Bem-vindo ao Cofrinho",
    "onb.welcome.desc":
      "Vamos configurar sua experiência em menos de 2 minutos.",
    "onb.skip": "Pular",
    "onb.next": "Continuar",
    "onb.back": "Voltar",
    "onb.finish": "Concluir",

    "layout.bar": "Pesquisar transações, metas, carteiras…",

    "common.search": "Pesquisar…",
    "common.save": "Salvar",
    "common.cancel": "Cancelar",
    "common.delete": "Excluir",
    "common.edit": "Editar",
    "common.new": "Novo",
    "common.viewAll": "Ver tudo",
    "common.month": "Mês",
    "common.category": "Categoria",
    "common.status": "Status",
    "common.date": "Data",
    "common.amount": "Valor",
    "common.wallet": "Carteira",
    "common.priority.high": "Alta",
    "common.priority.medium": "Média",
    "common.priority.low": "Baixa",
    "common.pt-en": "pt-BR",
    "common.export": "Exportar",

    "nf.title": "Parece que essa página sumiu.",
    "nf.desc": "O link pode estar quebrado ou a página foi movida.",
    "nf.back": "Voltar ao painel",
    "nf.home": "Ir para o início",
    "nf.error": "Ocorreu um erro ao carregar a página.",
    "nf.something.went.wrong":
      "Algo deu errado. Você pode tentar novamente ou voltar para a página inicial.",
    "nf.try.again": "Tentar novamente",
    "nf.go.home": "Ir para a página inicial",

    "error.title": "Ocorreu um erro",
    "error.desc": "Algo deu errado. Tente novamente ou volte para o painel.",
    "error.retry": "Tentar novamente",

    "dash.loading": "Carregando seu painel...",
  },
  en: {
    "nav.features": "Features",
    "nav.pricing": "Pricing",
    "nav.faq": "FAQ",
    "nav.login": "Log in",
    "nav.signup": "Sign up",
    "hero.badge": "New · AI purchase simulator",
    "hero.title": "Plan before you spend.",
    "hero.subtitle":
      "Cofrinho helps you simulate purchases, forecast your money and decide with confidence. No hidden fees. No surprises.",
    "hero.cta": "Start planning smarter",
    "hero.cta2": "Watch demo",
    "hero.trust": "Trusted by 24,000+ people in 12 countries",
    "features.title": "Everything you need to decide with clarity",
    "features.subtitle": "Elegant tools to master your everyday money.",
    "feat.sim.title": "Purchase Simulator",
    "feat.sim.desc":
      "Ask “can I buy this?” and see the real impact on the next 6 months.",
    "feat.forecast.title": "Financial forecasting",
    "feat.forecast.desc":
      "Auto models project your future balance based on your habits.",
    "feat.tracking.title": "Expense tracking",
    "feat.tracking.desc":
      "Smart categorization, filters and personalized insights.",
    "feat.wishlist.title": "Wishlist",
    "feat.wishlist.desc":
      "Organize what you want and find the best time to buy.",
    "feat.analytics.title": "Deep analytics",
    "feat.analytics.desc":
      "Clear charts for income, expenses, categories and trends.",
    "feat.wallets.title": "Multiple wallets",
    "feat.wallets.desc": "Cards, accounts, investments and cash in one place.",

    "testimonials.quote1": "I never buy on impulse again.",
    "testimonials.designer": "Designer",
    "testimonials.quote2": "The simulator saved my trip.",
    "testimonials.engineer": "Engineer",
    "testimonials.quote3": "I finally understand where my money goes.",
    "testimonials.student": "Student",

    "pricing.title": "Simple, transparent, no gotchas",
    "pricing.free": "Free",
    "pricing.pro": "Pro",
    "pricing.team": "Family",
    "pricing.month": "/mo",
    "pricing.cta": "Get started",
    "pricing.1wallet": "1 wallet",
    "pricing.unlimitedTransactions": "Unlimited transactions",
    "pricing.basicSupport": "Basic support",
    "pricing.value1": "$ 2",
    "pricing.value2": "$ 6",
    "pricing.unlimitedWallets": "Unlimited wallets",
    "pricing.advancedSimulator": "Advanced AI simulator",
    "pricing.unlimitedGoals": "Unlimited goals",
    "pricing.detailedAnalytics": "Detailed analytics",
    "pricing.upTo5People": "Up to 5 people",
    "pricing.sharedWallets": "Shared wallets",
    "pricing.familyReports": "Family reports",

    "faq.title": "Frequently asked questions",
    "footer.tagline": "Smarter financial decisions, every day.",

    "auth.login.title": "Welcome back",
    "auth.login.subtitle": "Log in to continue planning",
    "auth.signup.title": "Create your account",
    "auth.signup.subtitle": "Start free, no credit card required",
    "auth.forgot.title": "Reset password",
    "auth.forgot.subtitle": "We'll email you a reset link",

    "auth.email": "Email",
    "auth.password": "Password",
    "auth.name": "Name",
    "auth.namePlaceholder": "Mary Jane",
    "auth.emailPlaceholder": "mary.jane@example.com",
    "auth.passwordPlaceholder": "At least 8 characters",
    "auth.passwordHint": "Use 8+ characters with a mix of letters and numbers.",
    "auth.remember": "Remember me",
    "auth.forgot": "Forgot password?",
    "auth.login.button": "Log in",
    "auth.signup.button": "Create account",
    "auth.forgot.button": "Send link",
    "auth.have": "Already have an account?",
    "auth.dont": "Don't have an account?",
    "auth.or": "or continue with",

    "auth.title": "Balance",
    "auth.quote":
      "The purchase simulator turned my anxious shopping decisions into calm, confident ones.",
    "auth.loved": "Loved by 24,000+ mindful spenders",
    "auth.security": "Secured with end-to-end encryption",

    "auth.terms":
      "By creating an account, you agree to our Terms and Privacy Policy.",
    "auth.validation.email": "Enter a valid email",
    "auth.validation.password": "Password must be at least 8 characters",
    "auth.validation.name": "Enter your name",
    "auth.validation.match": "Passwords do not match",
    "auth.resetSent": "Link sent! Check your email.",

    // Onboarding
    "onboarding.skip": "Skip introduction",
    "onboarding.step": "Step",
    "onboarding.of": "of",
    "onboarding.welcome.title": "Welcome to Cofrinho",
    "onboarding.welcome.subtitle":
      "Let's set up your account in a few quick steps so you can start planning better.",
    "onboarding.welcome.start": "Let's get started",
    "onboarding.language.title": "Choose your language",
    "onboarding.language.subtitle": "You can change this later in settings.",
    "onboarding.currency.title": "Select your currency",
    "onboarding.currency.subtitle":
      "We'll use this currency throughout the app.",
    "onboarding.wallet.title": "Create your first wallet",
    "onboarding.wallet.subtitle": "A wallet helps you organize your money.",
    "onboarding.wallet.name": "Wallet name",
    "onboarding.wallet.namePlaceholder": "E.g. Main account",
    "onboarding.wallet.type": "Type",
    "onboarding.balance.title": "What's your starting balance?",
    "onboarding.balance.subtitle":
      "Tell us how much you have in this wallet now.",
    "onboarding.card.title": "Add a card",
    "onboarding.card.subtitle": "Optional — you can add cards later.",
    "onboarding.card.number": "Last 4 digits",
    "onboarding.card.limit": "Limit",
    "onboarding.income.title": "What's your monthly income?",
    "onboarding.income.subtitle": "This helps us make accurate forecasts.",
    "onboarding.goal.title": "Create your first goal",
    "onboarding.goal.subtitle": "What would you like to achieve?",
    "onboarding.goal.name": "Goal name",
    "onboarding.goal.namePlaceholder": "E.g. Emergency fund",
    "onboarding.goal.target": "Target amount",
    "onboarding.finish.title": "All set!",
    "onboarding.finish.subtitle":
      "Your account is ready. Let's plan your purchases smartly.",
    "onboarding.finish.cta": "Go to dashboard",
    "onboarding.welcome.list1": "Simulate any purchase before you make it",
    "onboarding.welcome.list2": "Forecast your balance for the next months",
    "onboarding.welcome.list3": "Organize goals and wishes with clarity",
    "onboarding.step1.title": "Pick your language",
    "onboarding.step1.subtitle": "You can change it later",
    "onboarding.flag": "🇺🇸",
    "onboarding.flag.subtitle": "English (US)",
    "onboarding.step2.title": "Your primary currency",
    "onboarding.step2.subtitle": "Used across the platform",
    "onboarding.step3.title": "Create your first wallet",
    "onboarding.step3.subtitle": "An account or payment method",
    "onboarding.step3.name": "Name",
    "onboarding.step4.title": "Initial balance",
    "onboarding.step4.subtitle": "How much do you have today?",
    "onboarding.step5.title": "Add a card (optional)",
    "onboarding.step5.subtitle": "So we can track your spend",
    "onboarding.step5.nickname": "Nickname",
    "onboarding.step5.number": "Last 4",
    "onboarding.step5.limit": "Limit",
    "onboarding.step6.title": "Your monthly income",
    "onboarding.step6.subtitle": "Average of recent weeks",
    "onboarding.step7.title": "Your first goal",
    "onboarding.step7.subtitle": "Let's help you get there!",
    "onboarding.step7.name": "Goal name",
    "onboarding.step7.fund": "Emergency fund",
    "onboarding.step7.target": "Target",
    "onboarding.step7.targetDate": "Target date",
    "onboarding.step8.title": "All set!",
    "onboarding.step8.subtitle": "Welcome to Cofrinho.",
    "onboarding.step8.open":
      "We'll open your dashboard and give you a quick tour of the features.",
    "onboarding.save": "Saving...",

    // Tour
    "tour.skip": "Skip tour",
    "tour.restart": "Restart tour",
    "tour.dashboard.title": "This is your dashboard",
    "tour.dashboard.desc":
      "See a complete overview of your finances in one place.",
    "tour.wallets.title": "Your wallets",
    "tour.wallets.desc": "Manage cards, cash, savings and investments.",
    "tour.simulator.title": "Purchase simulator",
    "tour.simulator.desc":
      "The heart of Cofrinho: find out if you can afford something.",
    "tour.wishlist.title": "Wishlist",
    "tour.wishlist.desc": "Save products and find the best time to buy them.",
    "tour.analytics.title": "Analytics",
    "tour.analytics.desc":
      "Understand your financial habits with detailed charts.",
    "tour.goals.title": "Financial goals",
    "tour.goals.desc": "Set objectives and track your progress.",

    // Dashboard
    "dashboard.greeting": "Good morning,",
    "dashboard.subtitle": "Here's a summary of your finances today.",
    "dashboard.snapshot": "Here's your snapshot",
    "dashboard.balance": "Current balance",
    "dashboard.income": "Monthly income",
    "dashboard.expenses": "Monthly expenses",
    "dashboard.savings": "Savings",
    "dashboard.healthScore": "Financial health",
    "dashboard.healthExcellent": "Excellent",
    "dashboard.healthGood": "Good",
    "dashboard.healthFair": "Fair",
    "dashboard.healthPoor": "Needs attention",
    "dashboard.quickActions": "Quick actions",
    "dashboard.addIncome": "Add income",
    "dashboard.addExpense": "Add expense",
    "dashboard.transfer": "Transfer",
    "dashboard.simulate": "Simulate purchase",
    "dashboard.cardsOverview": "Your cards",
    "dashboard.walletsOverview": "Wallets",
    "dashboard.recentTransactions": "Recent transactions",
    "dashboard.upcomingPurchases": "Upcoming purchases",
    "dashboard.monthlyForecast": "Monthly forecast",
    "dashboard.forecastDesc":
      "Projection of your balance over the coming months",
    "dashboard.vsLastMonth": "vs. last month",
    "dashboard.projection": "Projection for the next 12 months",
    "dashboard.basedOn": "Based on 12 signals",
    "dashboard.emergencyFund": "Emergency fund",
    "dashboard.spendControl": "Spend control",
    "dashboard.diversification": "Diversification",
    "dashboard.language": "en-US",

    // Wallets
    "wallets.title": "Wallets",
    "wallets.subtitle": "Manage all your accounts in one place.",
    "wallets.add": "New wallet",
    "wallets.total": "Total net worth",
    "wallets.type.credit": "Credit card",
    "wallets.type.debit": "Debit card",
    "wallets.type.cash": "Cash",
    "wallets.type.savings": "Savings",
    "wallets.type.investment": "Investments",
    "wallets.balance": "Balance",
    "wallets.limit": "Limit",
    "wallets.used": "used",
    "wallets.available": "Available",
    "wallets.confirmDelete": "Are you sure you want to delete this wallet?",
    "wallets.deleteFailed": "Could not delete the wallet.",

    // Transactions
    "transactions.title": "Transactions",
    "transactions.subtitle": "Your complete financial history.",
    "transactions.add": "New transaction",
    "transactions.searchPlaceholder": "Search transactions...",
    "transactions.type": "Type",
    "transactions.income": "Income",
    "transactions.expense": "Expense",
    "transactions.category": "Category",
    "transactions.wallet": "Wallet",
    "transactions.date": "Date",
    "transactions.status": "Status",
    "transactions.amount": "Amount",
    "transactions.description": "Description",
    "transactions.completed": "Completed",
    "transactions.pending": "Pending",
    "transactions.scheduled": "Scheduled",
    "transactions.noResults": "No transactions found",
    "transactions.income.added": "Income added",
    "transactions.expense.added": "Expense added",
    "transactions.transfer.added": "Transfer completed",
    "transactions.saved": "Transaction saved",
    "transactions.new": "New transaction",
    "transactions.register": "Record an income, expense or transfer",
    "transactions.emergency": "Emergency fund",
    "transactions.groceries": "Groceries",
    "transactions.obs": "Notes",
    "transactions.opt": "Optional",
    "transactions.transfer.recurring": "Recurring transfer",
    "transactions.transaction.recurring": "Recurring transaction",
    "transactions.recurring.description": "Repeat every month",
    "transactions.confirmDelete":
      "Are you sure you want to delete this transaction?",
    "transactions.deleteFailed": "Could not delete the transaction.",
    "transactions.movement": "All of your money movements in one place",

    "category.food": "Food",
    "category.transport": "Transport",
    "category.housing": "Housing",
    "category.entertainment": "Entertainment",
    "category.health": "Health",
    "category.shopping": "Shopping",
    "category.education": "Education",
    "category.salary": "Salary",
    "category.freelance": "Freelance",
    "category.investment": "Investment",
    "category.subscription": "Subscriptions",
    "category.other": "Other",

    "sidebar.dashboard": "Dashboard",
    "sidebar.transactions": "Transactions",
    "sidebar.wallets": "Wallets",
    "sidebar.wishlist": "Wishlist",
    "sidebar.simulator": "Simulator",
    "sidebar.analytics": "Analytics",
    "sidebar.goals": "Goals",
    "sidebar.calendar": "Calendar",
    "sidebar.profile": "Financial Profile",
    "sidebar.notifications": "Notifications",
    "sidebar.settings": "Settings",
    "sidebar.faq": "FAQ",
    "sidebar.account": "Account",
    "sidebar.delete": "Delete account",
    "sidebar.logout": "Log out",

    "dash.balance": "Current balance",
    "dash.income": "Monthly income",
    "dash.expenses": "Monthly expenses",
    "dash.savings": "Savings",
    "dash.health": "Financial health",
    "dash.quick": "Quick actions",
    "dash.addIncome": "Income",
    "dash.addExpense": "Expense",
    "dash.transfer": "Transfer",
    "dash.simulate": "Simulate",
    "dash.recent": "Recent transactions",
    "dash.upcoming": "Upcoming purchases",
    "dash.forecast": "Monthly forecast",
    "dash.wallets": "Wallets",

    "sim.title": "Can I buy this?",
    "sim.product": "Product",
    "sim.price": "Price",
    "sim.installments": "Installments",
    "sim.method": "Payment method",
    "sim.card": "Card",
    "sim.cash": "Cash",
    "sim.date": "Desired date",
    "sim.run": "Run simulation",
    "sim.result": "Simulation result",
    "sim.describe":
      "Describe the purchase and we'll show the impact for the next 6 months.",
    "sim.recommend":
      "Recommendation: postpone to October. Your safety margin will be 42% higher.",

    "goals.title": "Financial goals",
    "goals.new": "New goal",
    "wishlist.title": "Wishlist",
    "wishlist.subTitle": "What you want, when it makes sense to buy",
    "wishlist.new": "Add item",
    "wishlist.buyNow": "Can buy now",
    "wishlist.wait": "Wait a bit",
    "wishlist.notRec": "Not recommended",
    "wishlist.run": "Run simulation",
    "wishlist.load": "Could not load your wishlist.",
    "wishlist.empty": "Your wishlist is empty",
    "wishlist.desc":
      "Add something you want to buy and find out when it makes sense to make the purchase.",

    "cal.title": "Financial calendar",
    "cal.add": "New reminder",
    "notif.title": "Notifications",
    "notif.markAll": "Mark all as read",
    "notif.empty": "You're all caught up.",
    "notif.viewAll": "View all",

    "settings.title": "Settings",
    "settings.profile": "Profile",
    "settings.appearance": "Appearance",
    "settings.notifications": "Notifications",
    "settings.language": "Language",
    "settings.theme": "Theme",
    "settings.danger": "Danger zone",
    "settings.deleteDesc":
      "This will permanently delete your account and all data.",
    "settings.deleteConfirm":
      "Are you sure you want to delete your account? This action cannot be undone.",
    "settings.deleteConfirmButton": "Yes, delete my account",
    "settings.deleteCancelButton": "No, keep my account",
    "settings.deleteSuccess": "Your account has been successfully deleted.",
    "settings.deleteError":
      "There was an error deleting your account. Please try again later.",
    "settings.adjust": "Tune Cofrinho to your taste",
    "settings.mode": "Light, dark or system",
    "settings.recommend": "Purchase recommendations",
    "settings.bills": "Upcoming bills",
    "settings.goals": "Goal progress",
    "settings.reports": "Monthly reports",

    "onb.welcome.title": "Welcome to Cofrinho",
    "onb.welcome.desc": "Let's set up your experience in under 2 minutes.",
    "onb.skip": "Skip",
    "onb.next": "Continue",
    "onb.back": "Back",
    "onb.finish": "Finish",

    "layout.bar": "Search transactions, goals, wallets…",

    "common.search": "Search…",
    "common.save": "Save",
    "common.cancel": "Cancel",
    "common.delete": "Delete",
    "common.edit": "Edit",
    "common.new": "New",
    "common.viewAll": "View all",
    "common.month": "Month",
    "common.category": "Category",
    "common.status": "Status",
    "common.date": "Date",
    "common.amount": "Amount",
    "common.wallet": "Wallet",
    "common.priority.high": "High",
    "common.priority.medium": "Medium",
    "common.priority.low": "Low",
    "common.pt-en": "en-US",
    "common.export": "Export",

    "nf.title": "Looks like this page went missing.",
    "nf.desc": "The link may be broken or the page might have been moved.",
    "nf.back": "Back to Dashboard",
    "nf.home": "Go Home",
    "nf.error": "An error occurred while loading the page.",
    "nf.something.went.wrong":
      "Something went wrong. You can try again or head back home.",
    "nf.try.again": "Try again",
    "nf.go.home": "Go home",

    "error.title": "An error occurred",
    "error.desc":
      "Something went wrong. Please try again or go back to the dashboard.",
    "error.retry": "Try again",

    "dash.loading": "Loading your dashboard...",
  },
} as const;

export type TranslationKey = keyof (typeof translations)["pt" | "en"];
