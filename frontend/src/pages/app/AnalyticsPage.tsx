...
export default function AnalyticsPage() {
  ...
  return (
    <AppLayout title={t("sidebar.analytics")}>
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-semibold tracking-tight">{t("sidebar.analytics")}</h2>
          <p className="text-sm text-muted-foreground">
            {locale === "pt" ? "Insights sobre seu dinheiro" : "Insights about your money"}
          </p>
        </div>

        <div className="grid gap-4 lg:grid-cols-3">
          <div className="card-elevated p-6 lg:col-span-2">
            <h3 className="text-sm font-semibold">{locale === "pt" ? "Receita vs Despesa" : "Income vs Expenses"}</h3>
          ...
          <div className="card-elevated p-6">
            <h3 className="text-sm font-semibold">{locale === "pt" ? "Categorias" : "Category breakdown"}</h3>
            <div className="mt-4 h-72">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={categoryData} dataKey="value" innerRadius={55} outerRadius={90} paddingAngle={3}>
                    {categoryData.map((c) => (
          ...
          <div className="card-elevated p-6 lg:col-span-2">
            <h3 className="text-sm font-semibold">{locale === "pt" ? "Evolução mensal" : "Monthly evolution"}</h3>
          ...
          <div className="card-elevated p-6">
            <h3 className="text-sm font-semibold">{locale === "pt" ? "Poupança (%)" : "Savings rate (%)"}</h3>
            <div className="mt-4 h-64">
              <ResponsiveContainer width="100%" height="100%">
