    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          {locale === "pt"
            ? "Uma visão profunda da sua vida financeira"
            : "A deep view of your financial life"}
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="card-elevated p-6 lg:col-span-2">
          <h3 className="text-sm font-semibold">
            {locale === "pt" ? "Insights pessoais" : "Personal insights"}
          </h3>
        ...
        <StatCard
          icon={PiggyBank}
          label={locale === "pt" ? "Taxa de poupança" : "Savings rate"}
          value="24.8%"
          sub="+8.2%"
        />
        <StatCard
          icon={TrendingUp}
          label={locale === "pt" ? "Receita média" : "Avg income"}
          value={formatCurrency(5750)}
          sub="+3.1%"
        />
        <StatCard
          icon={Activity}
          label={locale === "pt" ? "Despesa média" : "Avg expenses"}
          value={formatCurrency(4360)}
          sub="-1.8%"
        />
        <StatCard
          icon={WalletIcon}
          label={locale === "pt" ? "Patrimônio líquido" : "Net worth"}
          ...
          <div className="mt-4 space-y-3">
            {categoryData.map((c) => {
              const pct = Math.round(
                (c.value / categoryData.reduce((a, b) => a + b.value, 0)) * 100,
              );
              return (
                <div key={c.name}>
