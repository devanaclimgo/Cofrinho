...
export default function TransactionsPage() {
  ...
  return (
    <AppLayout title={t("sidebar.transactions")}>
      <div className="space-y-6">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <h2 className="text-2xl font-semibold tracking-tight">{t("sidebar.transactions")}</h2>
            <p className="text-sm text-muted-foreground">
              {locale === "pt" ? "Todas as suas movimentações em um só lugar" : "All of your money movements in one place"}
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" className="h-10 rounded-xl"><Download className="mr-2 h-4 w-4" />Export</Button>
        ...
        <div className="card-elevated p-4 sm:p-5">
          <div className="flex flex-wrap items-center gap-2">
            <div className="relative min-w-0 flex-1">
              ...
              <Input placeholder={t("common.search")} className="h-10 rounded-xl pl-9 border-transparent bg-muted/60" />
            </div>
            <Select>
              <SelectTrigger className="h-10 w-[140px] rounded-xl"><SelectValue placeholder={t("common.category")} /></SelectTrigger>
              <SelectContent>
            ...
            <Select>
              <SelectTrigger className="h-10 w-[140px] rounded-xl"><SelectValue placeholder={t("common.wallet")} /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">All</SelectItem>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
              <tr>
                ...
                <th className="px-5 py-3 text-left font-medium">{t("common.wallet")}</th>
              ...
              {transactions.map((tx) => (
                <tr key={tx.id} className="border-t border-border transition hover:bg-muted/30">
                  <td className="px-5 py-4 text-muted-foreground">
                    {new Date(tx.date).toLocaleDateString(locale === "pt" ? "pt-BR" : "en-US", { day: "2-digit", month: "short" })}
                  </td>
                  ...
                  <td className="px-5 py-4">
                    <Badge className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${
                      tx.status === "cleared" ? "bg-success/10 text-success" : "bg-warning/10 text-warning-foreground"
                    } hover:opacity-100`}>
                      {tx.status}
                    </Badge>
                  </td>
                  <td className={`px-5 py-4 text-right font-semibold ${tx.type === "income" ? "text-success" : "text-foreground"}`}>
        ...
        <div className="space-y-3 md:hidden">
          {transactions.map((tx) => (
            <div key={tx.id} className="card-elevated flex items-center gap-3 p-4">
