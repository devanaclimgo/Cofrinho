        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {locale === "pt" ? "Suas conquistas planejadas" : "Your planned achievements"}
            </p>
          </div>
          <Button className="h-10 rounded-xl"><Plus className="mr-2 h-4 w-4" />{t("goals.new")}</Button>
        </div>
              ...
              <div key={g.id} className="card-elevated p-6">
                <div className="flex items-start gap-4">
                  <div className="relative grid h-24 w-24 place-items-center">
                    ...
                    <div className="absolute inset-0 grid place-items-center text-xl">{g.emoji}</div>
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-3 text-sm">
                      <span className="font-semibold">{formatCurrency(g.current)}</span>{" "}
                      <span className="text-muted-foreground">/ {formatCurrency(g.target)}</span>
                    </div>
                    <div className="mt-2 text-xs text-muted-foreground">
                      {locale === "pt" ? "Contribuição mensal" : "Monthly contribution"}: {formatCurrency(g.monthly)}
                    </div>
                  </div>
                </div>
